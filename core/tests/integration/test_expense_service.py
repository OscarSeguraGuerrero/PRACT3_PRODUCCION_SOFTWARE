...
def test_remove_expense_reduces_total():
    ...
    """
    service = create_service()
    expense_libro = service.create_expense("Libro", 15, "", date.today())
    service.create_expense("Revista", 5, "", date.today())

    service.remove_expense(expense_libro.id)
...
def test_total_amount_after_removal():
    ...
    expense_cursos = service.create_expense("Cursos", 30, "", date.today())
    service.create_expense("Internet", 25, "", date.today())

    assert service.total_amount() == 55

    service.remove_expense(expense_cursos.id)
